...
def main():
    ...
    import numpy as np
    learner = sl.GD(maxiter,regu,stepSize,lamb,featureset)
    labels = []
    labels.append(learner['otrain'][:,0])
    labels.append(learner['ptrain'][:,0])
    match = sum(learner['otrain'][:,0] == learner['ptrain'][:,0])
    #print '*******************'
    ...
    predv = np.sign(np.dot(newdatafeatset,learner['w'])+learner['b'])
    match = sum(predv[:,0] == ori_v_label_bool[:,0])
    labels.append(ori_v_label_bool[:,0])
    labels.append(predv[:,0])
    #print '*******************'
    ...
    predt = np.sign(np.dot(newdatafeatset,learner['w'])+learner['b'])
