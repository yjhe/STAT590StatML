...
def perceptron(maxiter, featureset):
	...
	# mistake counter for each sample
	...
	while iterr<maxiter:
		...
		for inloop in range(newdatafeatset.shape[0]):
			print "inloop over samples, loop number: ", inloop
			cursmp = newdatafeatset[inloop]
			cury = ori_train_label_bool[inloop]
			if featureset == 1:
				predy = sum(alphaa*ori_train_label_bool*np.dot(newdatafeatset,cursmp.T).reshape((nsmp,1)))
			else:
				#dott = sparsedot(newdatafeatset,cursmp.T)
				dott = np.dot(newdatafeatset,cursmp.T)
				predy = sum(alphaa*ori_train_label_bool*dott.reshape((nsmp,1)))
			if cury != np.sign(predy):
				alphaa[inloop] = alphaa[inloop] + 1			
		iterr = iterr + 1
	ptrain = np.sign(np.sum(np.tile(alphaa*ori_train_label_bool,(1,nsmp))*np.dot(newdatafeatset,newdatafeatset.T),axis=0))
	return {'alphaa':alphaa, 'thres':thres, 'y':ori_train_label_bool, 'X':newdatafeatset, 'otrain':ori_train_label_bool, 'ptrain': ptrain}
